...
/// Looks for `usage` object with `total_tokens`, `input_tokens`/`prompt_tokens`,
/// `output_tokens`/`completion_tokens`, and `cost.total`.
fn extract_usage_from_response(body: &[u8]) -> (u64, f64) {
    ...
    // Try total_tokens first, then fall back to summing input + output
...
fn estimate_tokens_from_body(body: &[u8]) -> u64 {
    ...
    // Try to parse as JSON and look for explicit token counts or message content
    if let Ok(json) = serde_json::from_slice::<Value>(body) {
        // Check for explicit token count in the request
        if let Some(tokens) = json.get("estimated_tokens").and_then(|v| v.as_u64()) {
            return tokens;
        }

        // Check for messages array (OpenAI-style chat completion)
...
pub async fn quota_middleware(
    ...
    // Skip quota checks for health endpoints
    let path = request.uri().path();
    if path.starts_with("/health") {
        return next.run(request).await;
    }

    // Extract user_id and workspace_id from extensions
    let user_id = request
        .extensions()
        .get::<cog_core::Claims>()
        .map(|c| c.sub.clone())
    ...
    // Read and buffer the request body to estimate tokens
    ...
    // Buffer the response body to parse usage, then reconstruct the response.
    let (mut parts, body) = response.into_parts();
    let resp_bytes = match to_bytes(body, usize::MAX).await {
        Ok(b) => b,
        Err(_) => {
            // If we cannot read the response, fall back to estimated tokens
            let _ = manager
    ...
    // Finalize quota adjustment with actual usage
    let _ = manager
        .finalize(&user_id, ws_id_ref, estimated_tokens, actual_tokens)
        .await;

    // Optionally record cost in response headers for observability
    if actual_cost > 0.0 {
